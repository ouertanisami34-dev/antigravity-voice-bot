...
async def play_next(guild):
    ...
    if cur and loops.get(guild.id):
        clone = {k: v for k, v in cur.items()
                 if k not in ('start_time', 'pause_start', 'paused_duration')}
        q.appendleft(clone)
    if not q:
    ...
    try:
        ...
        if ch: await ch.send(embed=playing_embed(song))
    except Exception as e:
...
async def run_play(guild, query, requester="?", channel=None):
    ...
    await bot.change_presence(activity=discord.Activity(
        type=discord.ActivityType.listening, name=song['title'][:50]))
    if channel: await channel.send(embed=playing_embed(song))
    return song
...
async def cmd_skip(ctx):
    ...
    if not vc or not vc.is_playing():
        return await ctx.send("❌ Rien à skipper.")
...
async def cmd_pause(ctx):
    vc = ctx.voice_client
    if not vc or not vc.is_playing():
        return await ctx.send("❌ Rien en cours.")
    vc.pause()
    s = now_playing.get(ctx.guild.id)
    if s: s['pause_start'] = datetime.datetime.now()
...
async def cmd_resume(ctx):
    ...
    if not vc or not vc.is_paused():
        return await ctx.send("❌ Pas en pause.")
    s = now_playing.get(ctx.guild.id)
    if s and 'pause_start' in s:
...
async def cmd_stop(ctx):
    ...
    await bot.change_presence(activity=None)
    await ctx.send("⏹️ Arrêté et déconnecté.")
...
async def cmd_rm(ctx, index: int):
    ...
    if not (1 <= index <= len(q)):
        return await ctx.send(f"❌ Index entre 1 et {len(q)}.")
    lst = list(q)
...
async def cmd_shuffle(ctx):
    q = Q(ctx.guild.id)
    if len(q) < 2:
        return await ctx.send("❌ Pas assez de titres à mélanger.")
    lst = list(q)
    random.shuffle(lst)
    queues[ctx.guild.id] = collections.deque(lst)
    await ctx.send(f"🔀 File mélangée ! ({len(lst)} titres)")
